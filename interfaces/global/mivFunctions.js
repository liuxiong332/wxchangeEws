...
mivFunctions.prototype = {
	...
	decodeFolderSpecialChars: function _decodeFolderSpecialChars(aStr) {
		...
		var result = aStr.toString();
		result = result.replace(/(\x252F|\x2525)/g,
			this.doDecodeFolderSpecialChars);
		return result;
	},
	...
	safeGetCharPref: function(aBranch, aName,
		aDefaultValue, aCreateWhenNotAvailable) {
		if (!aBranch) {
	...
	safeGetBoolPref: function _safeGetBoolPref(aBranch, aName,
		aDefaultValue, aCreateWhenNotAvailable) {
		if (!aBranch) {
			//return aDefaultValue;
			var realBranche = this.getBranch(aName);
	...
	safeGetIntPref: function _safeGetIntPref(aBranch, aName,
		aDefaultValue, aCreateWhenNotAvailable) {
		if (!aBranch) {
			//return aDefaultValue;
			var realBranche = this.getBranch(aName);
	 ...
	 */
	getString: function(aBundleName, aStringName, aParams, aComponent) {
    try {
      if (!aComponent) {
          return "";
      }
      var propName = "chrome://" + aComponent + "/locale/" +
      	aBundleName + ".properties";
      var props = this.getStringBundle(propName);

      if (aParams && aParams.length) {
          return props.formatStringFromName(aStringName,
	...
	writeToLogFile: function _writeToLogFile(aString) {
		...
		var file = this.safeGetCharPref(prefB,
			"extensions.1st-setup.debug.file", "/tmp/exchangecalendar.log", true);
		if (file != "") {
	...
	splitOnCharacter: function _splitOnCharacter(aString,
		aStartPos, aSplitCharacter) {
		if (!aString) {
			return null;
		}
